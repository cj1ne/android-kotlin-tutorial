...
- **for**
  ...
  for문은 Java의 foreach문과 비슷하며 `..` 연산자, `in` 키워드, `downTo` 키워드, `step` 키워드로 배열이나 컬렉션을 순회할 수 있습니다. 
...
- **생성자**
  ...
  기본 생성자 이외에도 다른 생성자들을 만들기 위해서는 **constructor 키워드**를 사용하여 체인형식으로 메서드처럼 선언합니다.
